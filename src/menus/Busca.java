...
public class Busca extends JFrame {
	...
	public Busca() {
		...
		setIconImage(Toolkit.getDefaultToolkit().getImage(Busca.class.getResource("/img/icon-buscar.png")));
		...
		panel.addTab("Hóspedes", new ImageIcon(Busca.class.getResource("/img/pessoas.png")), tbHospedes, null);
		...
		logo.setIcon(new ImageIcon(Busca.class.getResource("/img/Ha-100px.png")));
			...
			public void mouseClicked(MouseEvent e) {
				...
				if (filaHospedes >= 0) {
					...
					if(confirmarh == JOptionPane.YES_OPTION){
						...
						JOptionPane.showMessageDialog(contentPane, "Hóspede excluído");
						limparTabela();
						preencherTabelaHospedes();
											}
				}else {
					JOptionPane.showMessageDialog(null, "Erro: realize uma busca e selecione uma fileira para excluir");
				}							
			}
		});
	   ...
	   List<Hospedes> hospedesLista = buscarHospedes();
		try {
			for (Hospedes hospede : hospedesLista) {
				modeloHospede.addRow(new Object[] { hospede.getId(), hospede.getNome(), hospede.getSobrenome(), hospede.getDataNascimento(), hospede.getNacionalidade(), hospede.getTelefone() });
			}
		} catch (Exception e) {
			throw e;
		}
	...
	private void atualizarHospede() {
		Optional.ofNullable(modeloHospede.getValueAt(tbHospedes.getSelectedRow(), tbHospedes.getSelectedColumn()))
				.ifPresentOrElse(filaHuesped -> {
					...
					this.hospedesController.atualizar(nome,sobrenome,dataNascimento, nacionalidade, telefone, id);
