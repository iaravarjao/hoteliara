...
private Connection connection;
	...
	public List<Hospedes> listarHospedes() {
		...
		try {
			String sql = "SELECT id_hospede, nome, sobrenome, dataNascimento, nacionalidade, telefone FROM hospedes";
	...
	public List<Hospedes> buscarId(String id) {
		...
		try {
			...
			String sql = "SELECT id_hospede, nome, sobrenome, dataNascimento, nacionalidade, telefone FROM hospedes WHERE id_hospede = ?";
	...
	public void deletar(Integer id) {
		try (PreparedStatement stm = connection.prepareStatement("DELETE FROM hospedes WHERE id_hospede = ?")) {
			stm.setInt(1, id);
			stm.execute();
		} catch (SQLException e) {
			throw new RuntimeException(e);
	...
	private void transformarResultSetEmHospede(List<Hospedes> reservas, PreparedStatement pstm) throws SQLException {
		try (ResultSet rst = pstm.getResultSet()) {
			while (rst.next()) {
				Hospedes hospedes = new Hospedes(rst.getInt(1), rst.getString(2), rst.getString(3), rst.getDate(4), rst.getString(5), rst.getString(6));
